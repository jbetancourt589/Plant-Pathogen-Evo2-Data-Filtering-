#!/usr/bin/env python3
"""
Compare OpenGenome/Evo2-filtered FASTA files, then recreate an
OpenGenome/Evo2-like FASTA from the original NCBI FASTA.

This third reproduction set does not include a John-filtered FASTA, so John
comparisons are reported as N/A unless a --john file is provided explicitly.
...
def write_comparison_report(
    ...
    with path.open("w", encoding="utf-8", newline="") as csv_file:
        ...
        if john_records is None:
            writer.writerow(
                {
                    "comparison": "n/a_vs_opengenome",
                    "record_id": "N/A",
                    "status": "not_applicable",
                    "left_name": "N/A",
                    "left_length": "",
                    "right_name": "opengenome",
                    "right_length": "",
                    "sequences_match": "",
                    "note": "Filtered FASTA not provided for this reproduction set",
                }
            )
        else:
            write_pairwise_rows(
...
def write_summary(
    ...
    with path.open("w", encoding="utf-8", newline="\n") as summary_file:
        ...
        if john_path is None:
            summary_file.write("- N/A filtered FASTA: N/A (not provided)\n")
        else:
        ...
        if john_stats is None:
            summary_file.write("- N/A filtered: not provided\n")
        else:
        ...
        summary_file.write("N/A vs OpenGenome/Evo2\n")
        if john_vs_opengenome is None:
            summary_file.write("- Comparison status: N/A (filtered FASTA not provided)\n\n")
        else:
        ...
        if john_stats is None:
            summary_file.write("- N/A output still contains N bases: N/A\n")
            summary_file.write("  N count: N/A\n")
        else:
        ...
        if john_stats is None:
            summary_file.write("- N/A non-ACGT count, including N: N/A\n")
        else:
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Optional John's filtered output FASTA. Default: N/A",
    )
    parser.add_argument(
        "--opengenome",
        type=Path,
